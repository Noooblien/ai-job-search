...
- **Permission allowlist**: `.grok/config.toml` pre-approves only the specific commands the workflow needs; the `security-guards` CI job fails any PR that widens it, adds package-manifest lifecycle scripts, or weakens the personal-data gitignore rules. Note the allowlist governs Bash commands - the model's native `web_fetch` / `web_search` tools are outside its reach, which is exactly why the instruction-level rules above exist.
