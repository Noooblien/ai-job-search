...
An AI-powered job application framework built on [Grok Build](https://grok.x.ai/). This fork is **Grok-native** (not Claude Code). Fill in your profile, and let Grok evaluate job postings, tailor your CV, write cover letters, and prepare you for interviews.

> Note: This is an independent project and is not affiliated with, endorsed by, sponsored by, or maintained by xAI. Grok and Grok Build are referenced only to describe the toolchain this workflow uses. The original upstream template was authored for Claude Code; this fork ports that methodology to Grok Build.
>
...
- [Grok Build](https://grok.x.ai/) (CLI / TUI). Portal search skills live under `.agents/skills/` and work from any agent that can run shell commands; full workflows are wired for Grok via `.grok/`.
- Python 3.10+
...
`/setup`, `/scrape`, and `/apply` form the core workflow. These commands extend it once your profile is in place:
...
- **`/tracker`** starts a localhost-only cockpit (`python3 tools/tracker_ui.py` → http://127.0.0.1:8765/) over the scrape inbox and application pipeline. Skip, restore, and “I applied” write `seen_jobs.json` / `job_search_tracker.csv`. It never submits forms to job portals.
...
```
ai-job-search/
├── CLAUDE.md                          # Main candidate profile + workflow rules (filename kept for auto-load)
├── AGENTS.md                          # Grok project rules / SSOT pointers
├── .grok/
│   ├── config.toml                    # Grok Build permission allowlist
│   ├── commands/
...
│   │   ├── tracker.md                 # /tracker localhost inbox + pipeline UI
...
│       ├── job-scraper/               # Job search orchestration (/scrape)
...
│   ├── tracker_ui.py                  # Localhost job tracker UI (127.0.0.1 only)
│   ├── tracker/                       # Tracker server, store, and static UI
│   ├── lint_skills.py                 # CI lint for skills, commands, config.toml
...
6. **Compile and inspect** both PDFs: lualatex for the CV, xelatex for the cover letter. Grok reads the rendered pages and iterates on the LaTeX until the CV is exactly 2 pages with no orphaned entry titles, and the cover letter is exactly 1 page with the signature visible and fonts consistent.
...
- **Drafter-reviewer separation.** The drafter writes; a second Grok agent (`spawn_subagent`), spawned with a fresh context, researches the company and critiques the drafts. The drafter then revises. This catches missed keywords, weak framing, and generic language that a single pass often leaves in.
...
- Grok-native fork of [MadsLorentzen/ai-job-search](https://github.com/MadsLorentzen/ai-job-search); runtime is [Grok Build](https://grok.x.ai/) by [xAI](https://x.ai/)
